...
//        Log.d ( TAG, "computeAxis; min: " + yMin + "; max: " + yMax );
    ...
    protected void computeAxisValues(float min, float max) {
        ...
        int labelCount = mYAxis.getLabelCount(); // 7
        double range = Math.abs(yMax - yMin); // 25
        ...
        if ( mYAxis.isCustomAxisValue () && !mYAxis.isShowOnlyMinMaxEnabled() && mYAxis.getCustomAxis().length > 0 ) {
          int n = mYAxis.getLabelCount();
          ...
          int i;
          for ( i = 0; i < n; ++i) {
//            Log.d ( TAG, "computeAxisValues values: " +  mYAxis.getCustomAxis() [i] );
            mYAxis.mEntries[i] = mYAxis.getCustomAxis() [i];
          }
        }
        else {
